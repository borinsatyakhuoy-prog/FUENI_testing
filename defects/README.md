...
| [keycloak-userinfo-cors-misconfiguration](keycloak-userinfo-cors-misconfiguration/README.md) | All 3 (Keycloak) | Low | Confirmed server-wide across all 3 realms (patient/doctor/admin), live regression test (currently failing, by design) | `userinfo` endpoint reflects any Origin + allows credentials with no allowlist, on a single shared server-level config; `account`/`token`/`logout` endpoints audited and confirmed NOT chainable into data leakage |
...
| [admin-audit-log-generic-admin-identity](admin-audit-log-generic-admin-identity/README.md) | Admin | Medium/High | Confirmed, reconfirmed still unfixed 2026-08-21, not yet automated | Audit log attributes actions to a generic "Secondary Admin" seat label (with history predating the actual account), not the real named individual, contradicting its own "Comptes nommés individuels" claim |
...
| [language-switch-discards-unsaved-edit-silently](language-switch-discards-unsaved-edit-silently/README.md) | Patient | Low | Confirmed, not yet automated | Switching language while a profile edit form (e.g. "Contact d'urgence") is open silently discards unsaved typed input via a full page navigation, with no "unsaved changes" warning |
| [dialog-close-does-not-return-focus-to-trigger](dialog-close-does-not-return-focus-to-trigger/README.md) | Patient | Low | Confirmed on all 4 `/fr/security` controls; confirmed absent on `/fr/my-profile`'s 2 | Closing any `/fr/security` inline form/dialog (export, password, email, phone) leaves focus on `<body>` instead of the trigger button; `/fr/my-profile`'s equivalent controls correctly restore focus, giving a working reference implementation to copy |
| [login-page-severe-latency-cliff-above-5-concurrent](login-page-severe-latency-cliff-above-5-concurrent/README.md) | Patient (shared staging host) | Medium | Confirmed across 3 independent runs, not a permanent test | Login page P90 latency jumps 62x-124x once concurrency exceeds ~5 (339ms -> 5.9s -> 16.2s at 5/10/15 concurrent) - a sharp cliff, not a gradual slope; flat ~15-20% error rate at every level suggests a load-independent cause, can't yet distinguish real backend capacity from Cloudflare anti-automation policy |
