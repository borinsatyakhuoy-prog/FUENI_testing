...
**Result:** 🟡 PARTIAL (wrong-type tested; infected-file not tested)
...
- **Wrong file type:** rejected - confirmed via network log that zero upload request was fired
  after selecting the `.txt` file. However, **no visible error message** appeared telling the
  user why the file wasn't accepted (the upload slot just silently stayed empty) - a UX gap.
  See `005-wrong-file-type-attempt.png`.
