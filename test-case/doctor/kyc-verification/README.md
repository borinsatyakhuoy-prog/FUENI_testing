...
evidence referenced below. Folders for 010/016/017 exist for structural consistency but hold no
...
| 010 | KYC draft saved and resumed later (cross-device) | 🟡 **PARTIAL, not fully verified** | [`010-save-draft-resume/`](010-save-draft-resume/) (no screenshot) | The "Enregistrer et reprendre plus tard" button is disabled on an empty form and becomes enabled once data is entered - confirmed structurally, but this session did not verify actual save-and-resume-on-a-different-session behavior (would need a second browser context resuming the same account mid-form). |
